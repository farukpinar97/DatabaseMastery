...
namespace DatabaseMastery.TransportMongoDb.Services.ShipmentTrackingServices
{
    public interface IShipmentTrackingService
    {
        ...
        // Belirli hareketi ID ile getir (güncelleme formu için)
        Task<ResultShipmentTrackingDto> GetTrackingByIdAsync(string trackingNumber, string trackingId);
        // Hareketi güncelle
        Task UpdateTrackingAsync(UpdateShipmentTrackingDto updateDto);
        // Hareketi sil
        Task DeleteTrackingAsync(string trackingNumber, string trackingId);
    }
}